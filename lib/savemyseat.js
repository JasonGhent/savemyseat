...
Savemyseat.prototype.monitor = function() {
  var monitoringDaemon = new MonitoringDaemon(this._nano.config.url, this._dbBackupRepo);
    ...
    .then(
      function success(designDoc) {
        ...
        if(designDoc.version !== REQUIRED_DESIGN_DOC_VERSION) {
          throw new DatabaseBackupSourceError(util.format('The design doc for %s is not at the correct version. Expected %s. Design Doc @ %s', self.name, REQUIRED_DESIGN_DOC_VERSION, designDoc.version));
        }
        return;
      }
    );
    ...
    .then(
      function success() {
        ...
        return couchUtils.updateDesignDoc(
          destDb, 
          SAVEMYSEAT_DESIGN_DOC_ID, 
          SAVEMYSEAT_DESIGN_DOC, 
          SAVEMYSEAT_DESIGN_DOC_NAME, 
          SAVEMYSEAT_DESIGN_DOC_VIEW_NAME
        );
      }
    )
    .then(
  ...
  return Q.nfcall(sourceDb.view, SAVEMYSEAT_DESIGN_DOC_NAME, SAVEMYSEAT_DESIGN_DOC_VIEW_NAME, { reduce: true })
    .then(
      function success(reduce) {
        ...
        return Q.nfcall(destDb.view, SAVEMYSEAT_DESIGN_DOC_NAME, SAVEMYSEAT_DESIGN_DOC_VIEW_NAME, { reduce: true });
      }
    )
    .then(
      function success(reduce) {
...
function BackupStateData() {
}

BackupStateData.createDefault = function() {
};

function BackupStateValue(value, toErrorState) {
  this._value = value;
  this._toErrorState = toErrorState;
  this._incidentId = null;
}

BackupStateValue.prototype.set = function(newValue) {
  var currentValue = this._value;

  var newState = this._toErrorState(newValue);
  var currentState = this._toErrorState(currentValue);
...
var defaultBackupStateData = { 
  isRunning: true,
  docWriteFailures: 0, 
  documentCountsDifference: 0, 
  incidentId: null 
};

var defaultBackupStateConfig = {
  documentCountsDifferenceThreshold: 100
};

function BackupState(config) {
  this._isRunning = true;
  this._docWriteFailures = 0;
  this._documentCountsDifference = 0;
  this._isInError = false;
  this._config = config || _.cloneDeep(defaultBackupStateConfig);
  this._uuid = uuid.v4();
}
...
BackupState.prototype.evaluateErrorState = function() {
  ...
  if(this._documentCountsDifference > this._config.documentCountsDifferenceThreshold) {
    isInError = true;
    errorMessages.push(util.format(
      'The source contains %d more documents than the backup which is above the threshold of %d',
      this._documentCountsDifference,
      this._config.documentCountsDifferenceThreshold
    ));
  }
...
/**
 * Converts backup state values to error state values
 *
 * Error state values:
 *
 *    true - means the backup state is in error
 *    false - means the backup state is not in error
 */
function BackupStateToErrorStateConverter(config) {
  this._config = config || _.cloneDeep(defaultBackupStateToErrorStateConverterConfig);
}

BackupStateToErrorStateConverter.prototype.isRunning = function(value) {
  var isError = !value;
  return {
    isError: !value,
...
BackupStateToErrorStateConverter.prototype.docWriteFailures = function(value) {
  return value > 0;
};

BackupStateToErrorStateConverter.prototype.documentCountsDifference = function(value) {
  return value > this._config.documentCountsDifferenceThreshold;
};

BackupStateToErrorStateConverter.prototype.convert = function(backupState, stateName) {
  return this[stateName](backupState[stateName]);
};
 ...
 */
function MonitoringDaemon(dbUrl, dbBackupRepo, checkIntervalMs, backupStateToErrorStateConverter) {
  this._dbUrl = dbUrl;
  this._dbBackupRepo = dbBackupRepo;
  this._timeout = null;
  this._checkIntervalMs = checkIntervalMs || 10000;

  this._backupStateToErrorStateConverter = backupStateToErrorStateConverter;
...
MonitoringDaemon.prototype.initializeBackupStates = function() {
  ...
  _.each(this._dbBackupRepo.getNames(), function(dbBackupName) {
    dbBackupStates[dbBackupName] = new BackupState(self._config);
  });
};
...
MonitoringDaemon.prototype.start = function() {
  ...
  function next(err) {
    ...
    self._timeout = setTimeout(checkStatus.bind(null, next), self._checkIntervalMs);
  }
